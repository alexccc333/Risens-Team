...
class Player {
	...
	const WAKANIM_URL = 'https://www.wakanim.tv/ru/v2/catalogue/embeddedplayer/';
	const WAKA_MARKER = 'waka';
	...
	public function loadVideo($noLog = false) {
		...
		if ($result) {
			...
			if (strpos($videoId, self::WAKA_MARKER) !== false) {
				$this->_loadedUrl = self::WAKANIM_URL . substr($videoId, 4);
				$this->_source = self::WAKA_MARKER;
			}
			else {
	...
	public function isWaka() {
		return $this->_source === self::WAKA_MARKER;
	}
}
